...
步骤：
1. 现在cas服务中心注册应用，获取clientId
2. 跳转cas认证中心，登录后重定向回到业务系统，拿到ticket
3. 调用后端服务，后端带上ticket去调用cas的校验接口，获取用户信息，并生成token
4. 前端拿到token，单点登录结束
...
- 因为服务器验证ticket不只是验证，还需要获取用户信息以及对用户信息加密，另外还要生成自己的token
- 那直接把用户信息给到前端，不需要再去验证ticket可以吗：服务器不验证ticket，那么前端就拿不到token，怎么登录？
