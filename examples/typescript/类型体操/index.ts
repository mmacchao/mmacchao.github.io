...
// 追加参数
type AppendParam<Fn extends (...args: any[]) => any, A> = Fn extends (...args: infer T) => infer R
  ? (...args: [...T, A]) => R
  : never
...
// 将联合类型转换为排列数组
type Permutation<UnionType, C = UnionType> = [UnionType] extends [never]
  ? []
  : UnionType extends any
  ? [UnionType, ...Permutation<Exclude<C, UnionType>>]
  : never
type pm = Permutation<'a' | 'b' | 'c'>
const pm1: pm = ['a', 'b', 'c']

// 字符串长度，转换为数组然后直接得length
type LenOfString<S extends string, T extends any[] = []> = S extends `${infer F}${infer R}`
  ? LenOfString<R, [F, ...T]>
  : T['length']
